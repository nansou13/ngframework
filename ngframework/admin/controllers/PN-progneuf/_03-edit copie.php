...
class APN03 extends Action {
	...
	function launch(Request $request, Response $response) {
		...
		if (isset($_POST['mandat3'])) {
			...
			$filePdf = $_SERVER["DOCUMENT_ROOT"] . '/data/waitlot/' . $lotComp->nom . '.pdf';
if(file_exists($filePdf)){

	//on le deplace
	//check si dossier existe
            if (!is_dir($_SERVER["DOCUMENT_ROOT"] . '/data/planLot/')) {
                mkdir($_SERVER["DOCUMENT_ROOT"] . '/data/planLot/');
            }
            if (!is_dir($_SERVER["DOCUMENT_ROOT"] . '/data/planLot/'.$_POST['mandat3'].'/')) {
                mkdir($_SERVER["DOCUMENT_ROOT"] . '/data/planLot/'.$_POST['mandat3'].'/');
            }
			$rename01 = rename($_SERVER["DOCUMENT_ROOT"] . '/data/waitlot/' . $lotComp->nom . '.pdf',$_SERVER["DOCUMENT_ROOT"] . '/data/planLot/'.$_POST['mandat3'].'/'.$lotComp->nom. '.pdf');
		...
		$urlAbs = $_SERVER["DOCUMENT_ROOT"] . '/_img/neufimg/' . $mandatID . '/';
