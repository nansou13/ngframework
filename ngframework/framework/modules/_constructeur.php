...
class constructeur {
    ...
    public function getFolderController($controller){
    //list les fichiers
        $repController = _PATH_CONTROLLERS_GENE;
        $dir = opendir($repController);

        while ($f = readdir($dir)) {
           if(is_file($repController.$f)) {
              echo "NomF : ".$f;
              echo "<br><br>";
           }elseif(is_dir($repController.$f) && $f != '.' && $f != '..'){
               ...
               if(strlen($controller) == 2){
                    if($controller == $miniControllerName){
                        //Controller trouvé
                        $GoodController = $f;
                    }
               }elseif(strlen($controller) > 2){
                    if($controller == $maxiControllerName){
                        //Controller trouvé
                        $GoodController = $f;
                    }
               }
    ...
    public function getFileMethode($controller, $methode){
        //list les fichiers
        $repController = _PATH_CONTROLLERS_GENE.$controller.'/';
        $dir = opendir($repController);
        ...
        while ($f = readdir($dir)) {
           if(is_file($repController.$f)) {
               ...
               //verifie si le lien est compressé
               if(strlen($methode) == 2){
                    if($methode == $miniMethodeName){
                        //Controller trouvé
                        $GoodMethode = $f;
                    }
               }elseif(strlen($methode) > 2){
                    if($methode == $maxiMethodeName){
                        //Controller trouvé
                        $GoodMethode = $f;
                    }
               }
