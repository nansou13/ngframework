		...
		if(_ULTRA_GLOBAL){
			...
			foreach(self::$js_files as $key => $value){
				if($value){
					...
					$name = $infoJs[1];
					switch($type){
						case 'include' : $jsGlobals .= "<script type=\"text/javascript\" language=\"javascript\" src=\""._URL."/LD/JS/include/".$name.'.js'."\"></script>\n";break;
						...
						case 'data' : $jsReste .= "<script type=\"text/javascript\" language=\"javascript\" src=\"/data/".$name.'.js'."\"></script>\n";break;
		...
		if(_ULTRA_GLOBAL){
			...
			foreach(self::$css_files as $key => $value){
				...
				if($value){
					...
					switch($type){
						case 'include' : $cssGlobals .= "<link href=\""._URL."/LD/CS/include/".$name.".css\" media=\"all\" rel=\"stylesheet\" type=\"text/css\" />\n";break;
						...
						case 'data' : $cssReste .= "<link href=\"/data/".$name.".css\" media=\"all\" rel=\"stylesheet\" type=\"text/css\" />\n";break;
